...
n_segment = 5      # temporal shift module shift length
class TemporalShift(nn.Module):
    def __init__(self, net, n_segment=5, n_div=5, inplace=False):
...
class Pose_Estimation(nn.Module):
    ...
    def __init__(self, net_dict, batch_norm=False):
        ...
        #print("init self.model0: ", self.model0)
        
        self.model0 = TemporalShift(self.model0, n_segment)
        
        self.model1_1 = self._make_layer(net_dict[1][0], batch_norm)
        self.model1_1 = TemporalShift(self.model1_1, n_segment)
        
        self.model1_2 = self._make_layer(net_dict[1][1], batch_norm)
        self.model1_2 = TemporalShift(self.model1_2, n_segment)
        
        self.model2_1 = self._make_layer(net_dict[2][0], batch_norm)
        self.model2_1 = TemporalShift(self.model2_1, n_segment)

        self.model2_2 = self._make_layer(net_dict[2][1], batch_norm)
        self.model2_2 = TemporalShift(self.model2_2, n_segment)

        self.model3_1 = self._make_layer(net_dict[3][0], batch_norm)
        self.model3_1 = TemporalShift(self.model3_1, n_segment)

        self.model3_2 = self._make_layer(net_dict[3][1], batch_norm)
        self.model3_2 = TemporalShift(self.model3_2, n_segment)

        self.model4_1 = self._make_layer(net_dict[4][0], batch_norm)
        self.model4_1 = TemporalShift(self.model4_1, n_segment)

        self.model4_2 = self._make_layer(net_dict[4][1], batch_norm)
        self.model4_2 = TemporalShift(self.model4_2, n_segment)

        self.model5_1 = self._make_layer(net_dict[5][0], batch_norm)
        self.model5_1 = TemporalShift(self.model5_1, n_segment)

        self.model5_2 = self._make_layer(net_dict[5][1], batch_norm)
        self.model5_2 = TemporalShift(self.model5_2, n_segment)

        self.model6_1 = self._make_layer(net_dict[6][0], batch_norm)
        self.model6_1 = TemporalShift(self.model6_1, n_segment)

        self.model6_2 = self._make_layer(net_dict[6][1], batch_norm)
        self.model6_2 = TemporalShift(self.model6_2, n_segment)
    ...
    def _make_layer(self, net_dict, batch_norm=False, last_activity=False):
        ...
        for i in range(length):
            ...
            #print("one_layer.keys(): ", one_layer.keys(),type(one_layer.keys()))
        ...
        if last_activity:
            ...
            conv2d = nn.Conv2d(in_channels=v[0], out_channels=v[1], kernel_size=v[2], stride=v[3], padding=v[4])
            layers += [conv2d]
        #print("layerrrrr: ", len(layers), layers[0])
    ...
    def forward(self, x, mask):
        ...
        #print("forward  after:", type(x), x.size(), type(out0), out0.size())
