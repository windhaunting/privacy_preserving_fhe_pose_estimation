...
class Without_Temporal_Test(object):
    ...
    """     
    def process(self, model, input_path):
    ...
    def process_one_frame(self, model, origin_img):
        ...
        for m in range(len(multiplier)):
            scale = 1.0 # multiplier[m]  # do not scale
...
if __name__ == '__main__':
    os.environ['CUDA_VISIBLE_DEVICES'] = '3'
    parser = argparse.ArgumentParser()
    parser.add_argument('--model', type=str, default='../output_models/openpose_coco_no_temporal_shift2_iter_70000.pth', help='path to the weights file')
