...
from pose_estimation_TenSEAL import EncPoseNet

from TenSEAL_Encode_ckks.encoded_net_builder import build_from_pytorch
from TenSEAL_Encode_ckks.crypto import crypto as cr
...
INPUT_FRM_WIDTH = 16   # 640       # input imgage width 
INPUT_FRM_HEIGHT = 16  # 320       # input imgage height 
...
class TestEncodedPoseEstimation(unittest.TestCase):
    def __init__(self):
        ...
        # set the scale
        self.context.global_scale = pow(2, bits_scale)
    ...
    def encrypt_one_image(self, img_arr):
        ...
        encrypted_image = cr.encrypt_matrix(self.context, img_arr)
    ...
    def construct_model(self,args):
        ...
        state_dict = torch.load(args.model).state_dict()  # .state_dict() # ['state_dict']
    ...
    def normalize(self, origin_img):
        ...
        origin_img -= 128.0  #  np.mean(origin_img)  # origin_img -= 128.0
        origin_img /= 256.0  # np.std(origin_img)   # origin_img /= 256.0
    ...
    def apply_multi_layers_from_pose_estimation_model(self, model):
        ...
        encoded_net = build_from_pytorch(self.context, all_nn_sequntial[:1])  # all_nn_sequntial[0:2])
    ...
    def  process_encoded_one_frame(self, model, origin_img):
        # original_model is the original pose estimation without encoded, trained without encryption
        # 
        ...
        print("normed_img: ", type(normed_img), normed_img.shape)
        for m in range(len(multiplier)):
            ...
            mask = np.ones((1, 1, normed_img.shape[0] // stride, normed_img.shape[1] // stride), dtype=np.float32)
            """
            
            # preprocess
            normed_img_gray = cv2.cvtColor(normed_img, cv2.COLOR_BGR2GRAY)

            #encry_img = ts.ckks_tensor(self.context, normed_img)
            #print("encry_img type: ", type(encry_img), encry_img.shape)
            
            # tranfere to torch
            input_img_torch = torch.Tensor(normed_img_gray)
            
            print("input_img size: ",  input_img_torch.shape, model.model0[0])
            kernel_shape = model.model0[0].kernel_size
            first_conv_stride = model.model0[0].stride[0]
            
            image_shape = input_img_torch.shape
            
            input_img_view = input_img_torch.view(image_shape[0], image_shape[1]).tolist() # input_img_torch.tolist()  # input_img_torch.view(image_shape[1], image_shape[2], image_shape[3]).tolist()  
            
            print("input_img_view size: ", image_shape, len(input_img_view))
            
            
            # Encoding and encryption
            x_enc, windows_nb = ts.im2col_encoding(
            self.context, input_img_view, kernel_shape[0],
            kernel_shape[1], first_conv_stride
            )
        
            # Encrypted evaluation
            enc_model = EncPoseNet(model) 
            ...
            mask= mask # self.encrypt_one_image(mask)  # no need to encrypt
            ...
            out2_vec_mask, out2_heat_mask = self.apply_models_to_image(model, input_img, mask_var)
            ...
            # get the features
            print("test process vec1, heat1: ", out2_vec_mask.size())
            # get the heatmap
            heatmap = out2_heat_mask.data.cpu().numpy()
            ...
            # get the paf
            paf = out2_vec_mask.data.cpu().numpy()
    ...
    def test_multiple_frames(self, agrs, video_frm_parent_dir, model, confg_est_frm_arr):
        ...
        for i, frm_path in enumerate(processed_frame_path_list):
            ...
            all_peaks, candidate, subset = self.process_encoded_one_frame(model, origin_img)


            """
            encry_img = ts.ckks_tensor(self.context, origin_img)
            print("encry_img type: ", type(encry_img))
            ...
            """
            encoded_net = self.apply_multi_layers_from_pose_estimation_model(model)
            print("encoded_net mask_var shape 1111: ", encoded_net[0])
